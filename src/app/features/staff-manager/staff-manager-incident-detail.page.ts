          ...
          <div class="flex flex-col gap-6 lg:col-span-8">
            ...
            @if (issue()!.imageUrls && issue()!.imageUrls.length > 0) {
              <div class="rounded-xl border border-[var(--color-outline-variant)] bg-[var(--color-surface-container-lowest)] p-6 shadow-sm">
                ...
                <div class="grid grid-cols-2 gap-4">
                  @for (image of getCitizenImages(); track $index) {
                    <div class="group relative aspect-video overflow-hidden rounded-lg">
                      ...
                      <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent p-3">
                        <p class="text-[11px] font-medium text-white truncate">{{ image.uploadedByName }}</p>
                        <p class="text-[10px] text-white/80">{{ image.uploadedByRole }}</p>
                        <p class="text-[10px] text-white/60">{{ formatDateTime(image.uploadedAt) }}</p>
                      </div>
                    </div>
                  }
                </div>
            ...
            @if (getStaffImages().length > 0) {
              <div class="rounded-xl border border-[var(--color-outline-variant)] bg-[var(--color-surface-container-lowest)] p-6 shadow-sm">
                ...
                <div class="grid grid-cols-2 gap-4">
                  @for (image of getStaffImages(); track $index) {
                    <div class="group relative aspect-video overflow-hidden rounded-lg">
                      <img
                        class="h-full w-full cursor-pointer object-cover transition-transform duration-300 group-hover:scale-105"
                        [src]="getImageUrl(image.url)"
        ...
        <div class="relative">
          ...
          @if (selectedImage()) {
            <div class="absolute bottom-4 right-4 rounded-lg bg-black/70 px-4 py-3 text-white backdrop-blur-sm">
              <p class="text-[13px] font-semibold">{{ selectedImage()!.uploadedByName }}</p>
              <p class="text-[11px] text-white/80">{{ selectedImage()!.uploadedByRole }}</p>
              <p class="text-[10px] text-white/60">{{ formatDateTime(selectedImage()!.uploadedAt) }}</p>
            </div>
          }
        </div>
      </div>
...
export class StaffManagerIncidentDetailComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  selectedImage = signal<ImageInfo | null>(null)
  ...
  openImageModal(image: ImageInfo): void {
  ...
  getCitizenImages(): ImageInfo[] {
    return this.issue()?.imageUrls?.filter(img => img.kind === 'citizen') ?? []
  }

  getStaffImages(): ImageInfo[] {
    return this.issue()?.imageUrls?.filter(img => img.kind === 'staff') ?? []
  }
